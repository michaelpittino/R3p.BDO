...
public:
	...
	void getBytes(uint8_t* buf, uint32_t index, uint32_t len);
	std::wstring getWString();
	std::wstring getWString(int size);
	...
	unsigned int getUInt(uint32_t index);
	uint64_t getLong();
	...
	void putWString(std::wstring b, uint32_t size);
	void putString(std::string b, uint32_t size);
	void putBytesFromFile(std::string file, int offset = 5);
	...
	void putEmptyBytes(uint32_t len);
	...
	void putUInt(unsigned int value, uint32_t index);
